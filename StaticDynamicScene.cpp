...
//	readBlocksCSBFile("blocks.csb");
	...
	listener->onTouchBegan = CC_CALLBACK_2(StaticDynamic::onTouchBegan, this);		//加入觸碰開始事件
	listener->onTouchMoved = CC_CALLBACK_2(StaticDynamic::onTouchMoved, this);		//加入觸碰移動事件
	listener->onTouchEnded = CC_CALLBACK_2(StaticDynamic::onTouchEnded, this);		//加入觸碰離開事件

	this->_eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);	//加入剛創建的事件聆聽器
	this->schedule(CC_SCHEDULE_SELECTOR(StaticDynamic::update));

    return true;
}

void StaticDynamic::readBlocksCSBFile(const std::string csbfilename)
{
	...
	for (size_t i = 1; i <= 4; i++)
	{
		...
		auto edgeSprite = dynamic_cast<Sprite*>(csbRoot->getChildByName(objname));
...
void StaticDynamic::readSceneFile(const std::string csbfilename)
{
	// 讀取並將所有的 block 圖形 設定成 EdgeShape
	auto csbRoot = CSLoader::createNode(csbfilename);
	csbRoot->setPosition(_visibleSize.width / 2.0f, _visibleSize.height / 2.0f);
	...
	for (size_t i = 1; i <= 4; i++)
	{
		...
		auto const edgeSprite = dynamic_cast<Sprite*>(csbRoot->getChildByName(objname));
		Size ts = edgeSprite->getContentSize();
		...
		modelMatrix.m[3] = pntLoc.x + loc.x; //設定 Translation，自己的加上父親的, (pntLoc = csbRoot->getPosition())
		modelMatrix.m[7] = pntLoc.y + loc.y; //設定 Translation，自己的加上父親的
	...
	for (size_t i = 1; i <= 2; i++)
	{
		...
		auto const rectSprite = dynamic_cast<Sprite*>(csbRoot->getChildByName(objname));
	...
	for (size_t i = 1; i <= 2; i++)
	{
		...
		const auto triSprite = dynamic_cast<Sprite*>(csbRoot->getChildByName(objname));
	...
	for (size_t i = 1; i <= 2; i++)
	{
		...
		const auto circleSprite = dynamic_cast<Sprite*>(csbRoot->getChildByName(objname));
...
void StaticDynamic::update(float dt)
{
