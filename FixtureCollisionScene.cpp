...
{
	return FixtureCollision::create();
}
...
//  For Loading Plist+Texture
	...
	listener->onTouchBegan = CC_CALLBACK_2(FixtureCollision::onTouchBegan, this);		//加入觸碰開始事件
	listener->onTouchMoved = CC_CALLBACK_2(FixtureCollision::onTouchMoved, this);		//加入觸碰移動事件
	listener->onTouchEnded = CC_CALLBACK_2(FixtureCollision::onTouchEnded, this);		//加入觸碰離開事件
...
//	bodyDef.position.Set(loc.x/ PTM_RATIO, loc.y/ PTM_RATIO); 
	...
	auto ballSprite = dynamic_cast<Sprite *>(_csbRoot->getChildByName("densityball1"));
	...
	ballSprite = dynamic_cast<Sprite*>(_csbRoot->getChildByName("densityball2"));
...
void FixtureCollision::setupSensorAndCollision()
{
	...
	// 設置可以產生球的按鈕
	auto btnSprite = dynamic_cast<Sprite*>(_csbRoot->getChildByName("releasingBall_Btn"));
	...
	_collisionSprite = dynamic_cast<Sprite*>(_csbRoot->getChildByName("collisionSprite"));
