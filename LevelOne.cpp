...
#endif
	//加入 CLevelOneContactListener
	_b2World->SetContactListener(&_contactListener);
	//
	auto listener = EventListenerTouchOneByOne::create();	//創建一個一對一的事件聆聽器
	listener->onTouchBegan = CC_CALLBACK_2(LevelOne::onTouchBegan, this);		//加入觸碰開始事件
	listener->onTouchMoved = CC_CALLBACK_2(LevelOne::onTouchMoved, this);		//加入觸碰移動事件
	listener->onTouchEnded = CC_CALLBACK_2(LevelOne::onTouchEnded, this);		//加入觸碰離開事件
...
void LevelOne::update(float dt)
{
	...
	if (_bSmoke) {
		...
		// 判斷延遲的時間是否滿足
		if (_fSmokeTimer > 0.4f) { //可以噴發，實現這次的噴發
			_fSmokeTimer = 0;
			for (int i = 0; i < 1; i++) {
				// 建立 Spark Sprite 並與目前的物體結合
				auto sparkSprite = Sprite::createWithSpriteFrameName("flare.png");
...
bool LevelOne::onTouchBegan(cocos2d::Touch* pTouch, cocos2d::Event* pEvent)//觸碰開始事件
{
...
void  LevelOne::onTouchMoved(cocos2d::Touch* pTouch, cocos2d::Event* pEvent) //觸碰移動事件
{
	Point touchLoc = pTouch->getLocation();
...
void  LevelOne::onTouchEnded(cocos2d::Touch* pTouch, cocos2d::Event* pEvent) //觸碰結束事件 
{
	Point touchLoc = pTouch->getLocation();
