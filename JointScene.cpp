...
//  For Loading Plist+Texture
	...
	listener->onTouchBegan = CC_CALLBACK_2(JointScene::onTouchBegan, this);		//加入觸碰開始事件
	listener->onTouchMoved = CC_CALLBACK_2(JointScene::onTouchMoved, this);		//加入觸碰移動事件
	listener->onTouchEnded = CC_CALLBACK_2(JointScene::onTouchEnded, this);		//加入觸碰離開事件

	this->_eventDispatcher->addEventListenerWithSceneGraphPriority(listener, this);	//加入剛創建的事件聆聽器
	this->schedule(CC_SCHEDULE_SELECTOR(JointScene::update));
...
void JointScene::setupDistanceJoint()
{
	...
	auto circleSprite = dynamic_cast<Sprite *>(_csbRoot->getChildByName("circle01_dist"));
...
void JointScene::setupPrismaticJoint()
{
	...
	auto frameSprite = dynamic_cast<Sprite*>(_csbRoot->getChildByName("frame01_pri"));
	Point loc = frameSprite->getPosition();
	Size size = frameSprite->getContentSize();
	b2BodyDef staticBodyDef;
	staticBodyDef.type = b2_staticBody;
	...
	auto circleSprite = dynamic_cast<Sprite*>(_csbRoot->getChildByName("circle01_pri"));
	loc = circleSprite->getPosition();
	size = circleSprite->getContentSize();
	...
	circleSprite = dynamic_cast<Sprite*>(_csbRoot->getChildByName("circle02_pri"));
	loc = circleSprite->getPosition();
	size = circleSprite->getContentSize();
...
void JointScene::setupPulleyJoint()
{
	...
	auto circleSprite = dynamic_cast<Sprite*>(_csbRoot->getChildByName("circle01_pul"));
	...
	circleSprite = dynamic_cast<Sprite*>(_csbRoot->getChildByName("circle02_pul"));
...
void JointScene::setStaticEdge() {
	...
	auto edgeSprite = _csbRoot->getChildByName("wall1_01_car");
...
//void JointScene::readBlocksCSBFile(const std::string& csbfilename)
//{
...
//}
//
//void JointScene::readSceneFile(const std::string &csbfilename)
//{
//	auto csbRoot = CSLoader::createNode(csbfilename);
//	csbRoot->setPosition(_visibleSize.width / 2.0f, _visibleSize.height / 2.0f);
//	addChild(csbRoot, 1);
...
void JointScene::update(float dt)
{
...
void JointScene::createStaticBoundary()
{
	...
	for (size_t i = 1; i <= 1; i++)
	{
		...
		std::ostringstream ostr;
		std::string objname;
		ostr << "wall1_0" << i ; objname = ostr.str();
